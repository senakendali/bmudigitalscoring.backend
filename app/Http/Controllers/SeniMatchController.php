    ...
    public function matchList(Request $request)
{
     ...
     * Definisi BYE: pada babak paling awal, untuk sebuah battle_group,
     * tepat satu sisi (blue/red) memiliki pemain real (team_member_* terisi).
     * - Jika hanya 1 entri (blue ATAU red) di babak awal -> BYE -> hide entri tersebut.
     * - Jika ada 2 entri tapi hanya salah satu sisi punya pemain -> BYE -> hide keduanya (blue & red) di babak awal.
     * - Jika dua sisi sama-sama kosong (TBD vs TBD) -> JANGAN di-hide (biar bisa dijadwalkan).
     * Babak selain babak paling awal TIDAK disentuh.
     */
    $battle = $matches->where('mode', 'battle');

    if ($battle->isNotEmpty()) {
        // Ambil babak paling awal per pool
        $minRoundByPool = $battle
            ->groupBy('pool_id')
            ->map(fn($col) => (int) $col->min('round'));

        // Helper: cek ada pemain real di sebuah baris match sesuai tipe
        $hasReal = function ($m) {
            if (!$m) return false;
        ...
        // Kelompokkan per (pool_id|battle_group) agar tidak tercampur antar pool
        $groups = $battle
        ...
        foreach ($groups as $key => $groupMatches) {
            ...
            // Ambil entri pada babak paling awal untuk battle_group ini
            $firstRoundEntries = $groupMatches
                ->filter(fn($m) => (int) $m->round === $earliestRound)
                ->values();

            if ($firstRoundEntries->isEmpty()) {
                continue;
            }

            // CASE A: Babak awal hanya punya 1 entri → BYE → hide entri tersebut
            if ($firstRoundEntries->count() < 2) {
                $idsToHide = $idsToHide->merge($firstRoundEntries->pluck('id'));
                continue;
            }

            // CASE B: Ada 2 entri → cari BLUE dan RED
            $blue = $firstRoundEntries->firstWhere('corner', 'blue');
            $red  = $firstRoundEntries->firstWhere('corner', 'red');

            // Kalau struktur tidak lengkap (misal cuma dapat BLUE), fallback cek real di yang ada saja
            if (!$blue || !$red) {
                $blueHasReal = $blue ? $hasReal($blue) : false;
                $redHasReal  = $red  ? $hasReal($red)  : false;

                // Hide hanya jika tepat satu sisi real (BYE)
                if (($blueHasReal xor $redHasReal)) {
                    $idsToHide = $idsToHide->merge($firstRoundEntries->pluck('id'));
                }
                continue;
            }
            ...
            // BYE = tepat satu sisi real → hide keduanya di babak awal
            // Jika dua-duanya kosong (TBD vs TBD) → jangan hide.
            if (($blueHasReal xor $redHasReal)) {
                $idsToHide = $idsToHide->merge($firstRoundEntries->pluck('id'));
            }
        }
    ...
    protected function generatePoolMode($validated)
{
    ...
    // Ambil peserta (DEDUP by team_member_id)
    $participants = \App\Models\TournamentParticipant::where('tournament_id', $validated['tournament_id'])
        ...
        ->unique('team_member_id')   // ← penting: hitung atlet unik
        ->values();
    ...
    $poolSize      = (int) ($validated['pool_size'] ?? request()->input('pool_size') ?? 0);
    ...
    // === Bentuk UNIT (tiap unit = 1 slot tampil) — dari peserta UNIK
    if ($teamSize === 1) {
    ...
    // === NORMALISASI & DEBUG setelah tahu jumlah unit
    $totalUnits = $units->count();
    Log::info('[SeniPool][DEBUG] incoming', [
        'validated_pool_size' => $validated['pool_size'] ?? null,
        'raw_pool_size'       => request()->input('pool_size'),
        'poolSize_before'     => $poolSize,
        'participants_count'  => $participants->count(), // sudah UNIK
        'totalUnits'          => $totalUnits,
        'route'               => request()->path(),
        'method'              => request()->method(),
    ]);

    // Normalisasi poolSize
    if ($poolSize <= 0) {
        $poolSize = $totalUnits;
    }
    if ($poolSize == $participants->count()) {
        $poolSize = $totalUnits; // selaraskan definisi agar pasti single-pool
    }
    ...
    $distributeToPools = function (\Illuminate\Support\Collection $unitList, int $poolSize) use ($alternateOrder) {
        ...
        $poolContingents = array_fill(0, $poolCount, []);
        $remaining = array_sum(array_map('count', $buckets));

        // Pass 1
        while ($remaining > 0) {
            $poolOrder = range(0, $poolCount-1);
            usort($poolOrder, fn($a,$b)=>count($pools[$a])<=>count($pools[$b]));
            $progress = false;
            ...
            foreach ($poolOrder as $pi) {
                ...
                $pickedCid = null;
                foreach ($buckets as $cid => $arr) {
                    if (!empty($arr) && !in_array($cid, $poolContingents[$pi], true)) { $pickedCid = $cid; break; }
                }
                if ($pickedCid === null) continue;
        ...
        return array_map(fn($arr) => $alternateOrder(collect($arr)->values()), $pools);
    };

    $usedMemberIds = [];

    // === SINGLE POOL kalau pool_size >= totalUnits
    if ($poolSize >= $totalUnits) {
        ...
        Log::info('[SeniPool] single-pool enforced', compact('poolSize','totalUnits'));
        return response()->json(['message' => 'Seni matches created (pool mode) successfully: single pool (no dummy).']);
    }

    // === Multi-pool
    $poolsUnits = $distributeToPools($units->values(), $poolSize);
   ...
   protected function generateBattleMode(array $validated)
{
    ...
    protected function generateBattleMode_with_dummy(array $validated)
    {
        // Hapus pool & match lama
        $oldPools = \App\Models\SeniPool::where('tournament_id', $validated['tournament_id'])
            ->where('match_category_id', $validated['match_category_id'])
        ...
        // --- siapkan contingent dummy & helper bikin tim dummy pakai cara existing ---
        $dummyContingents = [310,311,312,313,314,315];
        if (method_exists($this, 'ensureContingentsExist')) {
            $this->ensureContingentsExist($dummyContingents, $validated['tournament_id']);
        }

        /**
         * Buat 1 tim dummy via createDummySeniTeam(), lalu kembalikan array:
         * ['contingent_id' => X, 'members' => [id1,id2,(id3)]]
         * NOTE: Mengambil anggota dummy terbaru berdasar tournament/match/age/gender+contingent.
         */
        $buildDummyTeam = function (int $teamSize, int $contingentId, $templateMember = null) use ($validated) {
            // panggil util existing
            $this->createDummySeniTeam($teamSize, $contingentId, $templateMember, $validated);

            // ambil id anggota dummy yg barusan dibuat (paling baru)
            $memberIds = DB::table('tournament_participants as tp')
                ->join('team_members as tm', 'tp.team_member_id', '=', 'tm.id')
                ->where('tp.tournament_id', $validated['tournament_id'])
                ->whereNull('tp.pool_id') // belum di-assign pool (baru dibuat)
                ->where('tm.match_category_id', $validated['match_category_id'])
                ->where('tm.age_category_id',  $validated['age_category_id'])
                ->where('tm.gender',           $validated['gender'])
                ->where('tm.contingent_id',    $contingentId)
        ...
        if (($validated['bracket_type'] ?? null) === 'full_prestasi') {
            ...
            if ($currentTeamCount < 6) {
                ...
                for ($i = 0; $i < $neededTeams; $i++) {
                    $contingentId = $dummyContingents[$i % count($dummyContingents)];
                    $this->createDummySeniTeam($teamSize, $contingentId, $templateMember, $validated);
                }
        ...
        foreach ($pools as $i => $chunk) {
            ...
            // ===== CASE A: Pool cuma 1 tim -> buat DUMMY lawan (bukan auto-menang) =====
            if ($N === 1) {
                $onlyTeam = $chunk->values()->first();
                $templateMember = $onlyTeam['members'][0] ?? optional($participants->first())->participant;

                $contingentIdDummy = $dummyContingents[$i % count($dummyContingents)];
                $dummyTeam = $buildDummyTeam($teamSize, $contingentIdDummy, $templateMember);

                // assign anggota dummy ke pool ini
                \App\Models\TournamentParticipant::whereIn('team_member_id', $dummyTeam['members'])
                    ->where('tournament_id', $validated['tournament_id'])
                    ->update(['pool_id' => $pool->id]);

                [$m1, $m2, $m3] = $this->extractMemberIds($onlyTeam, $teamSize);
                $d1 = $dummyTeam['members'][0] ?? null;
                $d2 = $teamSize >= 2 ? ($dummyTeam['members'][1] ?? null) : null;
                $d3 = $teamSize >= 3 ? ($dummyTeam['members'][2] ?? null) : null;
                ...
                \App\Models\SeniMatch::create([
                    ...
                    'round_label'       => $roundLabel,
                    'corner'            => 'red',
                    'contingent_id'     => $dummyTeam['contingent_id'],
                    'team_member_1'     => $d1,
                    'team_member_2'     => $teamSize >= 2 ? $d2 : null,
                    'team_member_3'     => $teamSize >= 3 ? $d3 : null,
                    'status'            => 'not_started',
                ]);
            ...
            for ($b = 0; $b < $byeCount && $bag->count() > 0; $b++) {
                ...
                if ($targetPairs === 1) {
                    // FINAL LANGSUNG & satu sisi kosong → ganti BYE dengan tim dummy
                    $templateMember   = $team['members'][0] ?? optional($participants->first())->participant;
                    $contingentIdDummy = $dummyContingents[($i + $b) % count($dummyContingents)];
                    $dummyTeam         = $buildDummyTeam($teamSize, $contingentIdDummy, $templateMember);
                    ...
                    $pairings[] = $byeBlueSide ? [$team, $dummyTeam] : [$dummyTeam, $team];
                } else {
                    // BYE normal untuk non-final
                    $pairings[] = $byeBlueSide ? [$team, null] : [null, $team];
                }
            ...
            foreach ($pairings as [$blueTeam, $redTeam]) {
                ...
                if ($blueTeam !== null && $redTeam === null) {
                    ...
                    $blueMatch = \App\Models\SeniMatch::create([
                        ...
                        'team_member_3'     => $teamSize >= 3 ? $b3 : null,
                        'status'            => 'not_started',
                        'winner_corner'     => ($targetPairs === 1 ? null : 'blue'), // final langsung -> jangan auto-menang
                    ]);

                    // Kalau final langsung, isi sisi RED dengan dummy sekarang juga
                    if ($targetPairs === 1) {
                        $templateMember   = $blueTeam['members'][0] ?? optional($participants->first())->participant;
                        $contingentIdDummy = $dummyContingents[$i % count($dummyContingents)];
                        $dummyTeam         = $buildDummyTeam($teamSize, $contingentIdDummy, $templateMember);
                        ...
                        \App\Models\SeniMatch::create([
                            ...
                            'corner'            => 'red',
                            'contingent_id'     => $dummyTeam['contingent_id'],
                            'team_member_1'     => $dummyTeam['members'][0] ?? null,
                            'team_member_2'     => $teamSize >= 2 ? ($dummyTeam['members'][1] ?? null) : null,
                            'team_member_3'     => $teamSize >= 3 ? ($dummyTeam['members'][2] ?? null) : null,
                            'status'            => 'not_started',
                        ]);
                    }
                ...
                if ($blueTeam === null && $redTeam !== null) {
                    ...
                    if ($targetPairs === 1) {
                        $templateMember   = $redTeam['members'][0] ?? optional($participants->first())->participant;
                        $contingentIdDummy = $dummyContingents[$i % count($dummyContingents)];
                        $dummyTeam         = $buildDummyTeam($teamSize, $contingentIdDummy, $templateMember);
            ...
            while (count($currentRound) > 1) {
                ...
                for ($j = 0; $j < count($currentRound); $j += 2) {
                    ...
                    // Prefill jika parent BYE (kode asli)
                    $prevRound = $round - 1;
        ...
        return response()->json(['message' => 'Battle matches generated (dummy dibuat saat pool 1 tim / final langsung tanpa lawan).']);
    }

    protected function generateBattleMode_mantep(array $validated)
    {
        // Hapus pool & match lama
        $oldPools = \App\Models\SeniPool::where('tournament_id', $validated['tournament_id'])
            ->where('match_category_id', $validated['match_category_id'])
    ...
    public function regeneratePool(Request $request, \App\Models\SeniPool $pool = null)
{
    ...
    public function getParticipantCounts(Request $request)
    {
        $request->validate([
            'tournament_id'    => 'required|exists:tournaments,id',
            'age_category_id'  => 'required|exists:age_categories,id',
            'match_category_id'=> 'required|in:2,3,4,5', // 2/5=Tunggal, 3=Ganda, 4=Regu (sesuai mapping lu)
        ]);
        ...
        // Ambil peserta turnamen sesuai filter (distinct by team_member_id untuk jaga-jaga)
        $base = TournamentParticipant::query()
            ->where('tournament_id', $tournamentId)
            ->whereHas('participant', function ($q) use ($ageCategoryId, $matchCategoryId) {
                $q->where('age_category_id', $ageCategoryId)
                ->where('match_category_id', $matchCategoryId);
            })
            ->with(['participant' => function ($q) {
                $cols = ['id','gender','contingent_id','age_category_id','match_category_id'];
                if (Schema::hasColumn('team_members','category_class_id')) $cols[] = 'category_class_id';
                $q->select($cols);
            }])
            ->get()
            ->filter(fn($tp) => !is_null($tp->participant))
            ->unique('team_member_id')            // pastikan unik per TM
            ->values();

        // Normalisasi gender → 'male' | 'female' | null
        $normalizeGender = function ($raw) {
            if ($raw === null) return null;
            $s = strtolower(trim($raw));
            // Map umum: m/f, l/p, laki/perempuan, male/female
            if (in_array($s, ['m','l','male','laki','laki-laki','pria','cowok'], true)) return 'male';
            if (in_array($s, ['f','p','female','perempuan','wanita','cewek'], true))    return 'female';
            return $s === 'male' || $s === 'female' ? $s : null;
        };

        // Group by gender normalized
        $groupedByGender = $base->groupBy(function ($tp) use ($normalizeGender) {
            return $normalizeGender($tp->participant->gender ?? null);
        });
        ...
        foreach (['male','female'] as $gender) {
            ...
            $rows = $groupedByGender->get($gender, collect());

            if (in_array($matchCategoryId, [2,5], true)) {
                // Tunggal → 1 orang = 1 unit penampilan
                $result[$gender] = $rows->count();
            } else {
                // Ganda / Regu
                // - Ganda (3)  : butuh 2 orang per kontingen → jumlah tim = floor(count/2)
                // - Regu  (4)  : butuh 3 orang per kontingen → jumlah tim = floor(count/3)
                $required = $matchCategoryId === 3 ? 2 : 3;

                $perContingentCounts = $rows
                    ->groupBy(fn($tp) => $tp->participant->contingent_id)
                    ->map(function ($group) use ($required) {
                        // hitung orang unik di kontingen ini
                        $count = $group->unique('team_member_id')->count();
                        return intdiv($count, $required); // floor(count/required)
                    });

                $result[$gender] = $perContingentCounts->sum();
            }
        }
