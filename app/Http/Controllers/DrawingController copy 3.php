    ...
    public function getPools(Request $request)
{
    ...
    // Filter jika tournament_id tersedia
    if ($filterTournamentId) {
        $poolQuery->where('tournament_id', $filterTournamentId);
    }

    /** @var \Illuminate\Support\Collection<int,\App\Models\Pool> $pools */
    $pools = $poolQuery->get();
    ...
    // Siapkan daftar kunci untuk agregasi per turnamen+match_category
    ...
    $byClass = DB::table('tournament_participants as tp')
        ...
        ->get()
        // → map: [$tid][$matchCatId][$classId] = count
        ->reduce(function ($carry, $row) {
            $carry[$row->tournament_id][$row->match_category_id][$row->category_class_id] = (int)$row->c;
            return $carry;
        }, []);
    ...
    $byAge = DB::table('tournament_participants as tp')
        ...
        ->get()
        // → map: [$tid][$matchCatId][$ageId] = count
        ->reduce(function ($carry, $row) {
            $carry[$row->tournament_id][$row->match_category_id][$row->age_category_id] = (int)$row->c;
            return $carry;
        }, []);
    ...
    $pools = $pools->map(function ($pool) use ($byClass, $byAge) {
        ...
        if ($classId) {
            ...
            $available = $byAge[$tid][$matchCatId][$ageCategoryId] ?? 0;
        }
